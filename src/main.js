...
function navMetaHtml() {
  const live = getSyncStatus().live
    ? ` <span class="nav-live">Live</span>`
    : "";
  return `${relativeTime(getState().updatedAt)}${live}`;
}
    ...
    <div class="screen">
      <header class="nav">
        ...
        <div class="nav-copy">
          ...
          <div class="nav-meta">What’s out · Smyrna</div>
        </div>
        <div class="nav-aside">
          ${getSyncStatus().live ? `<span class="nav-live">Live</span>` : `<span class="nav-live off">Offline</span>`}
          <span class="nav-updated" data-updated>${esc(relativeTime(getState().updatedAt).replace(/^Updated /, ""))}</span>
        </div>
      </header>
      <div class="status-row">
        ...
        <div class="status-cell gf">
          <span class="status-kicker">100% GF</span>
